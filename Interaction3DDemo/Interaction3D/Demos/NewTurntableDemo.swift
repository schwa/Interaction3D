...
/// Turntable rotation via the new gesture manager. Drag → yaw/pitch, accumulated as quaternion.
/// Handles yaw/pitch decomposition internally since quaternion rotation isn't additive.
private struct NewTurntableDragModifier: ViewModifier {
...
private struct NewPanDragModifier: ViewModifier {
    let modifiers: EventModifiers?
    let sensitivity: CGFloat
...
private extension View {
    func newTurntableDrag(_ modifiers: EventModifiers? = nil, sensitivity: CGFloat = 0.005, rotation: Binding<simd_quatf>) -> some View {
        modifier(NewTurntableDragModifier(modifiers: modifiers, sensitivity: sensitivity, rotation: rotation))
    }

    func newPanDrag(_ modifiers: EventModifiers? = nil, sensitivity: CGFloat = 0.01, target: Binding<SIMD3<Float>>) -> some View {
        modifier(NewPanDragModifier(modifiers: modifiers, sensitivity: sensitivity, target: target))
    }
}
...
struct NewTurntableDemo: View {
    ...
    var body: some View {
        ...
        #if os(macOS)
        .newScrollGesture(transformer: ScalingTransformer(magnitude: -0.01), writes: $distance)
        #endif
        .newMagnifyGesture(transformer: ScalingTransformer(magnitude: -1.0), writes: $distance)
...
extension NewTurntableDemo: DemoView {
    static var metadata = DemoMetadata(
        name: "New Turntable",
        description: "Turntable camera using the new gesture manager.",
        group: "Interaction"
    )
}
