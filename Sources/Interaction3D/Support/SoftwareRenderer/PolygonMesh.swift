...
struct MeshRenderState {
    var rearFaces: [PolygonMesh.Face] = []
    var frontFaces: [PolygonMesh.Face] = []
    var rearEdges: Set<PolygonMesh.Edge> = []
    var frontEdges: Set<PolygonMesh.Edge> = []
    var rendererContext = SoftwareRendererContext()

    mutating func update(mesh: PolygonMesh, rotation: simd_quatf, size: CGSize, verticalFOV: Double) {
        rearFaces = []
        frontFaces = []
        rearEdges = []
        frontEdges = []
